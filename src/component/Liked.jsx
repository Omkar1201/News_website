        ...
        <div>
           
            {
                liked_article.map((data, index) => (
                    <div key={index} className='liked items-center overflow-auto mt-5 flex border-2 rounded-md relative pb-5 hover:shadow-xl transition-all duration-[0.5s]'>
                        <img className=' rounded-t-md max-h-[12rem]' alt='' src={data.Image} />
                        <div className='px-2 flex justify-between w-full'>
                            <div>
                                <div className='text-sm font-bold'>Source: {data.Source}</div>
                                <div className='text-xs'>Published at: {data.PublishedOn.substring(0, 10)} Time: {data.PublishedOn.substring(11, 16)}</div>
                                <div className=' text-[0.9rem] font-bold my-2'>{data.Title}</div>
                                {/* <div>{data.data.content}</div>  */}
                                <div className='mb-7 text-blue-700 text-[0.85rem] font-semibold'>{data.Summary.substr(0,500)}{data.Summary.length > 500 ? <>...</> :<></>}</div>
