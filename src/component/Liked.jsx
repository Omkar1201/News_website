...
export default function Liked() {
    ...
    function handledelete(index) {
        const updatedLikedArticles = liked_article.filter((article, i) => i !== index);
        setLikedArticles(updatedLikedArticles);
    
        const deletedArticle = liked_article[index];
        if (deletedArticle) {
      ...
      if (liked_article.length === 0) {
        return <div className='flex justify-center items-center text-2xl font-bold h-screen flex-col gap-2'>No result<Link to='/'><span className=' py-1 bg-blue-400 font-normal border-black hover:bg-blue-500 cursor-pointer rounded-lg px-2 text-white'>Go to Home</span></Link></div>;
      }
                ...
                liked_article.map((data, index) => (
                    <div key={index} className='liked overflow-auto mt-5 flex border-2 rounded-md relative pb-5 hover:shadow-xl transition-all duration-[0.5s]'>
                        ...
                        <div className='px-2 flex justify-between w-full'>
                            ...
                            <div onClick={() => handledelete(index)} className='text-2xl h-fit rounded-full p-1  cursor-pointer hover:bg-red-200 '><MdOutlineDelete /></div>
                        </div>
                    </div>
                ))
            }
